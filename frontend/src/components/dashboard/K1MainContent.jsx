/**
 * K1MainContent Component
 * Main content area showing documents for selected phase or overview
 */

import { useMemo, useState } from 'react';
import { K1_TIMELINE, OPTIONAL_DOCS, K1_GUIDANCE, CONDITIONAL_DOCS } from '../../data/k1Guidance';
import DocumentCard from './DocumentCard';
import TimelineView from './TimelineView';

// Phase 1 contains the long lead-time documents (priority)
const PHASE_1_DOCS = K1_TIMELINE[0]?.docsNeeded || [];

// Documents that need to be mailed (from phases 1-4)
const MAILING_DOCS = [
...
// Mailing Checklist Component for Phase 6
function MailingChecklist({ documents, onOpenPanel }) {
  // Track which items have been placed in the packet (separate from document completion)
  ...
  // Required and conditional items always show
  const requiredItems = MAILING_DOCS.filter(d => d.required);
  const conditionalItems = MAILING_DOCS.filter(d => d.conditional);

  // Optional items only show if they're completed on previous pages
  const completedOptionalItems = MAILING_DOCS.filter(d => {
  ...
  // Count required documents that are completed (ready to be mailed)
  ...
  const ChecklistItem = ({ item }) => {
    ...
    return (
      ...
      >
        {/* Checkbox - clicking toggles "in packet" status */}
        <button
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="p-4">
          ...
          >
            How to organize and mail your I-129F petition
          </p>
          <p
      ...
      {/* Completed optional documents - only show if user completed them on Strengthen Your Case page */}
    ...
    >
      ...
      >
        <div>
          ...
          >
            Include these with your I-129F mailing to make your case stronger
          </p>
        </div>
        <svg
      ...
      {isExpanded && (
        <div className="mt-4 space-y-4">
          {documents.map(doc => {
            const guidance = K1_GUIDANCE[doc.document_name];
            return (
              <div key={doc.id}>
                <DocumentCard
                  document={doc}
                ...
                {guidance?.strengthening_reason && (
                  ...
                  >
                    Why it helps: {guidance.strengthening_reason}
                  </p>
                )}
              </div>
            );
...
export default function K1MainContent({
  activePhase,
  documents,
  commentCounts = {},
  onOpenPanel,
  ...
  const phaseConfig = useMemo(() => {
    ...
    return K1_TIMELINE.find(p => p.phase === phaseNum);
  }, [activePhase]);
  ...
  const overviewData = useMemo(() => {
    ...
    const requiredDocs = documents.filter(d => !OPTIONAL_DOCS.includes(d.document_name));
    const optionalDocs = documents.filter(d => OPTIONAL_DOCS.includes(d.document_name));
      ...
      <div className="space-y-8">
        ...
        {overviewData.longLeadTimeDocs.length > 0 && (
          ...
          >
            ...
            >
              <strong>These documents have long lead times</strong> — they can take weeks or even months to obtain. Police certificates from foreign countries often take the longest. Start gathering these now so they don't delay your petition later.
            </p>
            <div className="space-y-3">
  ...
  // Render Mail Your Petition phase with special mailing checklist
  if (phaseConfig && phaseConfig.phase === 6) {
    return (
      <div className="space-y-6">
        {/* Phase header */}
        <div>
        ...
        {/* Mailing Checklist */}
        <MailingChecklist documents={documents} onOpenPanel={onOpenPanel} />
      </div>
    );
  }
